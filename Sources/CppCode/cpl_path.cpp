 ...
 * </pre>
 *
 * @param filePath the filename potentially including a path.
 *
...
const char *CPLGetPath( const char *filePath )

{
    int         iFileStart = CPLFindFilenameStart(filePath);
    ...
    strncpy( pszStaticResult, filePath, iFileStart );
...
const char *CPLGetDirname( const char *filePath )
 ...
 * system in use.  
 *
 * @param filePath the filename with path to test.
 *
 * @return TRUE if the filename is relative or FALSE if it is absolute. 
 */

int CPLIsFilenameRelative( const char *filePath )

{
    if( (strlen(filePath) > 2 && strncmp(filePath+1,":\\",2) == 0)
        || filePath[0] == '\\'
        || filePath[0] == '/' )
        return FALSE;
    else
