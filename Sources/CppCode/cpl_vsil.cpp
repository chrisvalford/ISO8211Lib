...
int VSIUnlink( const char * filePath )

{
    VSIFilesystemHandler *poFSHandler = 
        VSIFileManager::GetHandler( filePath );

    return poFSHandler->Unlink( filePath );
}
 ...
 *
 * @param filePath the path of the filesystem object to be queried.
 * @param psStatBuf the structure to load with information. 
 *
 * @return 0 on success or -1 on an error.
 */

int VSIStatL( const char * filePath, VSIStatBufL *psStatBuf )

{
    VSIFilesystemHandler *poFSHandler = 
        VSIFileManager::GetHandler( filePath );

    return poFSHandler->Stat( filePath, psStatBuf );
}
...
FILE *VSIFOpenL( const char * filePath, const char * pszAccess )

{
    VSIFilesystemHandler *poFSHandler = 
        VSIFileManager::GetHandler( filePath );

    return (FILE *) poFSHandler->Open( filePath, pszAccess );
}
