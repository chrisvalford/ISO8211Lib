 ...
 *
 * @param filePath the filename to be created.
...
FILE *VSIFileFromMemBuffer( const char *filePath, 
                          GByte *pabyData, 
...

{
    ...
    return (FILE *) poHandler->Open( filePath, "r+" );
}
 ...
 *
 * @param filePath the name of the file to grab the buffer of.
 * @param pnDataLength length returned in this variable.
...
GByte *VSIGetMemFileBuffer( const char *filePath, 
...

{
    ...
    if( poHandler->oFileList.find(filePath) == poHandler->oFileList.end() )
        return NULL;

    VSIMemFile *poFile = poHandler->oFileList[filePath];
    GByte *pabyData;
    ...
    if( bUnlinkAndSeize )
    {
        ...
        delete poFile;
        poHandler->oFileList.erase( poHandler->oFileList.find(filePath) );
    }
